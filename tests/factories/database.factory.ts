import { IPaginationOptions, IPaginatedResult, IFilterOptions, IOrderByOptions } from '@/types/database.types';
import { IBaseEntity } from '@/models/entities/base.entity';
import { BaseFactory } from './base.factory';

export class DatabaseFactory {
  static createPaginationOptions(overrides: Partial<IPaginationOptions> = {}): IPaginationOptions {
    return {
      page: 1,
      limit: 10,
      ...overrides,
  ...
  static createFilterOptions(overrides: Partial<IFilterOptions> = {}): IFilterOptions {
    return {
      where: {},
      ...overrides,
    };
  ...
  static createOrderOptions(overrides: Partial<IOrderByOptions> = {}): IOrderByOptions {
    return {
      column: 'created_at',
      ascending: false,
      ...overrides,
  ...
  static createPaginatedEntities(
    count: number,
    options: IPaginationOptions,
    entityFactory: (index: number) => IBaseEntity = (index) => BaseFactory.createBaseEntity({ id: `entity-${index}` })
  ): IPaginatedResult<IBaseEntity> {
    const entities = Array.from({ length: count }, (_, index) => entityFactory(index));
  ...
  static createDatabaseConstraintError() {
    return this.createSupabaseError('duplicate key value violates unique constraint', 'PGRST002');
  }
}